...
import java.util.*;		//자바 컬랙션도 util안에 들어 있다
public class Sample1 {
	public static void main(String[] args) {
		//컬랙션
		//자료구조를 이용할 수 있도록 구현된 일종의 프레임워크 -> 사용방법이 딱 정해져 있다
		//자바 컬랙션은 interface로 구성되어 있어 바로 객체로 조립하여 사용할 수는 없다
		...
		List L=new ArrayList();	//컬랙션은 interface라서 컬랙션이 구현된 클래스를 이용해서 객체를 만들어야 한다
		//List.add(자료) 	: 해당 자료를 리스트에 추가함. 가장 마지막 번호에 추가됨
		//List.add(index, 자료) : 해당 index에 자료를 추가함. 뒤 자료부터는 하나씩 index가 밀림
		//List.ger(index)	: index에 해당하는 자료를 확인
		L.add(1);
		L.add(2);
		L.add(3);
		L.add(3);
		...
		//List는 배열과 달리 size()메서드로 크기를 확인해야 한다. length를 사용할 수 없다***
